...
def load_state():
    storage.init()
    cfg = storage.get_meta("config", {})
    for key, value in DEFAULT_CONFIG.items():
        cfg.setdefault(key, value)
    return {
        "config": cfg,
        "update_cursor": storage.get_meta("update_cursor", 0),
        "alerted": storage.get_meta("alerted", {}),
        "prune": storage.get_meta("prune", []),
        "chat_ids": _targets(),
    }


def save_state(state):
    storage.set_meta("config", state["config"])
    storage.set_meta("update_cursor", state["update_cursor"])
    storage.set_meta("alerted", state["alerted"])
    storage.set_meta("prune", state["prune"])
...
def nt_send_photo(chat_id, png_bytes, caption=""):
    ...
    try:
        ...
        return _msg_id(resp.json())
    except requests.RequestException as exc:
        print(f"messenger sendPhoto failed: {exc}")
        return None


def nt_delete(chat_id, message_id):
    nt_call("deleteMessage", chat_id=chat_id, message_id=message_id)
...
def _queue(state, chat_id, message_id):
    if message_id:
        state["prune"].append([str(chat_id), message_id])


def reply(state, chat_id, text):
    _queue(state, chat_id, nt_send(chat_id, text))


def run_prune(state):
    for chat_id, message_id in state.get("prune", []):
        nt_delete(chat_id, message_id)
    state["prune"] = []
...
def process_commands(state, offers):
    ...
    for upd in updates:
        ...
        _queue(state, chat_id, msg.get("message_id"))
        ...
        if cmd in ("/start", "/help"):
            reply(state, chat_id, HELP_TEXT + "\n\n" + fmt_config(cfg))

        elif cmd == "/setprice":
            try:
                cfg["max_price_per_1k"] = round(float(args[0].replace("$", "")), 4)
                changed = True
                reply(state, chat_id, "\u2705 " + fmt_config(cfg))
            except (IndexError, ValueError):
                reply(state, chat_id, "Usage: /setprice 4.80")
            ...
            try:
                ...
                reply(state, chat_id, "Usage: /setminorder 1000")

        elif cmd in ("/enable", "/resume"):
            cfg["enabled"] = True
            changed = True
            reply(state, chat_id, "\u2705 Alerts resumed.\n" + fmt_config(cfg))

        elif cmd in ("/disable", "/pause"):
            cfg["enabled"] = False
            changed = True
            reply(state, chat_id, "\u23f8 Alerts paused.")
            ...
            if within:
                ...
                reply(state, chat_id, "Cheapest offers within your min-order filter:\n\n" + "\n\n".join(lines))
            else:
                reply(state, chat_id, "No offers found within your min-order filter.")
            ...
            if top:
                ...
                reply(state, chat_id, "Couldn't fetch the recommended offer right now.")

        elif cmd in ("/graphhelp", "/helpgraph"):
            reply(state, chat_id, GRAPH_HELP)

        elif cmd == "/graph":
            handle_graph(state, chat_id, args, cfg)

        else:
            reply(state, chat_id, "Unknown command.\n\n" + HELP_TEXT)
...
def effective_price_per_1k(listed_ppk, delivery_method, order_qty):
    ...
    qty = order_qty if order_qty and order_qty > 0 else 0
    received = max(qty * keep, 1000.0)
    eff = listed_ppk * (1.0 + FEE_RATE) / keep
    eff += FEE_FLAT * 1000.0 / received
    return round(eff, 4)
...
def record_history(offers, cfg, top=None):
    ...
    storage.append_sample(now.isoformat(), summary, HISTORY_RETENTION_DAYS)
    print("Recorded history point.")
...
def _mark_last(ax, xs, ys, color):
    x, y = xs[-1], ys[-1]
    ax.scatter([x], [y], s=46, color=color, edgecolor="#0d1117", zorder=5)
    ax.annotate(
        f"${y:,.2f}", (x, y), textcoords="offset points", xytext=(8, 6),
...
def render_graph(opts):
    ...
    if opts["show"]["min"]:
        ...
        if xs:
            ax.plot(
                xs, ys, color=accent, linewidth=2.6, solid_capstyle="round",
                marker="o", markersize=3.5, markerfacecolor=accent,
                markeredgecolor="#0d1117", label="Cheapest" + cost_tag + suffix,
            )
            ax.fill_between(xs, ys, min(ys), color=accent, alpha=0.10)
            _mark_last(ax, xs, ys, accent)
            plotted = True
    if opts["show"]["rec"]:
        xs, ys = series(rec_key)
        if xs:
            ax.plot(
                xs, ys, color=second, linewidth=2.2, solid_capstyle="round",
                marker="o", markersize=3.0, markerfacecolor=second,
                markeredgecolor="#0d1117", label="Source pick" + cost_tag,
            )
            _mark_last(ax, xs, ys, second)
            plotted = True
    threshold = None if eff else opts.get("threshold")
    if opts["show"]["threshold"] and threshold is not None:
        ax.axhline(
            threshold, color="#f85149", linestyle=(0, (6, 4)),
            linewidth=1.6, label=f"Alert threshold ${threshold:.2f}",
        )
        plotted = True
    ...
    title = "Real cost" if eff else "Listed price"
    ax.set_title(
        f"{title} per 1,000 units  \u00b7  {opts['label']}",
        fontsize=15, fontweight="bold", color="#e6edf3", loc="left", pad=14,
    )
    ax.set_ylabel("USD / 1,000" + (" (after tax + fees)" if eff else ""), fontsize=10)
    ax.grid(True, axis="y", color="#21262d", linewidth=1.0)
...
def handle_graph(state, chat_id, args, cfg):
    ...
    try:
        ...
        reply(state, chat_id, "Sorry, I couldn't render that graph. Try a different range.")
        return
    if result is None:
        reply(
            state,
        ...
        return
    png, caption = result
    _queue(state, chat_id, nt_send_photo(chat_id, png, caption=caption))
